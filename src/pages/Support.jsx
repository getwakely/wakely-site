...
export default function Support() {
  ...
  async function handleSubmit(e) {
    ...
    setStatus('loading')
    try {
      const res = await fetch('https://ecduxhxhtsjtgeixhems.supabase.co/functions/v1/support', {
        method: 'POST',
    ...
    return (
      <div className="pt-24 pb-16 min-h-screen flex items-center justify-center px-6">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center max-w-md">
          ...
          <h1 className="text-3xl font-display font-semibold text-wakely-dark mb-3">Ticket Submitted</h1>
          <p className="text-wakely-dark/55 mb-2">Your support ticket has been successfully submitted. We apologize for any inconvenience.</p>
          <p className="text-wakely-dark/40 text-sm mb-8">Our team will review your request and get back to you within 24-48 hours.</p>
  ...
  return (
    <div className="pt-24 pb-16 min-h-screen">
      ...
      >
        ...
        <h1 className="text-4xl font-display font-semibold text-wakely-dark tracking-tight mt-6 mb-2">Need Help?</h1>
        <p className="text-wakely-dark/50 mb-8">Submit a support ticket and we'll get back to you as soon as possible.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input type="text" required placeholder="Subject" value={form.subject} onChange={e => setForm({ ...form, subject: e.target.value })}
            className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-white text-wakely-dark placeholder:text-wakely-dark/30 focus:outline-none focus:ring-2 focus:ring-wakely-blue/30 focus:border-wakely-blue transition-all" />
          <textarea required placeholder="Describe your issue or question" rows={5} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })}
